...
import TypeProduitService from '../services/TypeProduitService';
...
const Table = (props) => {
    ...
    const loadItems = () => {
        ProduitService.get((data, status) => {
                setLoading(false);
    ...
    const deleteItem = (item) => {
        setYesNo({   
            ...
            callback : ()=> {
                setLoading(true);
                ProduitService.delete(item.id, (data, status)=>{
                    setLoading(false);
                    loadItems();
                });
            },
    ...
    return (
        <>
            ...
            <DataTable dataKey="id" value={items} 
                ...
                <Column field="typeProduit.libelle" header="Type produit" sortable />
...
const Form = (props) => {
    ...
    useEffect(()=> {
        if(!visible) return;
        TypeProduitService.get((data)=> data && setTypeProduits(data), {size: pageMaxSize})
    }, [visible])
        ...
        setYesNo(
            {   
                ...
                callback : ()=> {
                    ...
                    if(data.id) ProduitService.update(data, onResponse); else ProduitService.add(data, onResponse);
                },
            }
        )
    ...
    return(
        <Dialog visible={visible} style={{ width: '800px' }} header="Détails du produit" modal className="p-fluid" 
            footer={
                <>
            ...
            >  
                ...
                <div className="field">
                    <label htmlFor="typeProduit">Type produit</label>
                    <Dropdown id="typeProduit" options={typeProduits} value={data?.typeProduit} onChange={bind}
