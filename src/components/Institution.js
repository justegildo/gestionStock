...
const Institution = () => {
    ...
    const deleteSelectedInstituts = () => {
            ...
            toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Institution supprimé', life: 3000 });
    }

    const onInputChange = (e, nomChefParc) => {
        const val = (e.target && e.target.value) || '';
        let _institut = { ...institut };
        _institut[`${nomChefParc}`] = val;

        setInstitut(_institut);
    }

    const onInputNumberChange = (e, nomChefParc) => {
        const val = e.value || 0;
        let _institut = { ...institut };
        _institut[`${nomChefParc}`] = val;
    ...
    const deleteInstitutsDialogFooter = (
        <>
            <Button label="Non" icon="pi pi-times" className="p-button-text" onClick={hideDeleteInstitutsDialog} />
            <Button label="Oui" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedInstituts} />
        </>
    );
            ...
            <div className="col-12">
                <div className="card">
                    ...
                    <DataTable ref={dt} value={instituts} selection={selectedInstituts} onSelectionChange={(e) => setSelectedInstituts(e.value)}
                        ...
                        <Column field="libelle" header="Libellé" syortable body={libelleBodyTemplate} headerStyle={{ width: '14%', minWidth: '10rem' }}></Column>
                        <Column field="nomChefParc" header="Nom Chef Parc" body={nomChefParcBodyTemplate} sortable headerStyle={{ width: '14%', minWidth: '8rem' }}></Column>
                        <Column field="prenomChefParc" header="Prénom chef parc" sortable body={prenomChefParcBodyTemplate} headerStyle={{ width: '14%', minWidth: '10rem' }}></Column>
                        <Column field="emailChefParc" header="Email Chef Parc" body={emailChefParcBodyTemplate} sortable headerStyle={{ width: '14%', minWidth: '10rem' }}></Column>
                        <Column field="contact" header="Contact" body={contactBodyTemplate} sortable headerStyle={{ width: '14%', minWidth: '10rem' }}></Column>
                        <Column field="nomResponsable" header="Nom responsable" body={nomResponsableBodyTemplate} sortable headerStyle={{ width: '14%', minWidth: '10rem' }}></Column>
                    ...
                    <Dialog visible={deleteInstitutDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteInstitutDialogFooter} onHide={hideDeleteInstitutDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {institut && <span>Voulez-vous vraiment supprimé l'institution <b>{institut.name}</b>?</span>}
                        </div>
                    </Dialog>

                    <Dialog visible={deleteInstitutsDialog} style={{ width: '450px' }} header="Confirmation" modal footer={deleteInstitutsDialogFooter} onHide={hideDeleteInstitutsDialog}>
                        <div className="flex align-items-center justify-content-center">
                            <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                            {institut && <span>Voulez-vous vraiment supprimé les institutions sélectionnées ?</span>}
                        </div>
                    </Dialog>
                </div>
            </div>
...
export default React.memo(Institution, comparisonFn);