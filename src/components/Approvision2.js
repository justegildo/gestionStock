...
const Form = (props) => {
  ...
  /*   const [ligneApprovisionnement, setLigneApprovisionnement] = useState(null);

    useEffect(()=> {
        if(!visible) return;
        LigneApprovisionnement.get((data)=> data && setLigneApprovisionnement(data), {size: pageMaxSize})
    }, [visible])
     */
    const submit = () => {
        if(!data) return;
