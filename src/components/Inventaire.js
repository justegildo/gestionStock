...
import LigneInventaire from '../services/LigneInventaireService';
...
const Table = (props) => {
    ...
    const loadItems = () => {
        InventaireService.get((data, status) => {
                setLoading(false);
    ...
    const deleteItem = (item) => {
        setYesNo({   
            ...
            callback : ()=> {
                setLoading(true);
                InventaireService.delete(item.id, (data, status)=>{
                    setLoading(false);
                    loadItems();
                });
            },
...
const Form = (props) => {
    ...
    const [ligneInventaire, setLigneInventaire] = useState(null);

    useEffect(()=> {
        if(!visible) return;
        LigneInventaire.get((data)=> data && setLigneInventaire(data), {size: pageMaxSize})
    }, [visible])
        ...
        setYesNo(
            {   
                ...
                callback : ()=> {
                    ...
                    if(data.id) InventaireService.update(data, onResponse); else InventaireService.add(data, onResponse);
                },
            }
        )
    ...
    return(
        <Dialog visible={visible} style={{ width: '800px' }} header="Détails de l'Inventaire" modal className="p-fluid" 
            footer={
                <>
            ...
            >  
               ...
               {/*  <div className="field">
                    ...
                    <InputText id="contact" value={data && data.contact} onChange={bind} placeholder="ex: +229 98 00 00 00" />
                </div> */}
